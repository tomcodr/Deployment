...
export default {
  ...
  methods: {
    async fetchBrakeTemperatureValue() {
      try {
        const response = await fetch("https://cartrackerapi.onrender.com/api/v1/fahrzeuge/67c012ef-39f7-48c1-8d7a-092fcad45c08/messwerte?type=brakeSurfaceTemperaturefr");
        const data = await response.json();

        console.log("API Response:", data);

        if (Array.isArray(data.messwerte) && data.messwerte.length > 0) {
          const brakeTemperature = Math.round(data.messwerte[0]?.brakeSurfaceTemperaturefr);
